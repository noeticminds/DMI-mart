...
abstract class Abstract_Page {
	...
	private function prepare_upgrade_modal() {
		...
		if ( ! WP_Smush::is_pro() ) {
			$cta_url = $this->get_utm_link( array( 'utm_campaign' => 'smush_welcome_modal_preload_critical_images' ) );
		} elseif ( $has_lazy_preload_page ) {
			$cta_url = Helper::get_page_url( 'smush-lazy-preload&view=preload' );
		} else {
			$cta_url = '';
		}
